...
if (dirType != "none"){
    ...
    if (audioOut.match("HTTP")) {
        //Equalizer thru Web Audio API
        const frequencies = [40,121,547,674,818,1503,1605,1722,4160,12000];
        const ctx = new AudioContext();
        const filters = frequencies.map(freq => {
            const filter = ctx.createBiquadFilter();
            filter.type = "peaking";
            filter.frequency.value = freq;
            filter.Q.value = 1;      // Bandbreite
    ...
    function StartPlayRadioDir(index) {
        try {
            const response = fetch("/radioFavorites", {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ Playx: index })//+fileExt})
            });
            console.log("POST Playx: " + index)
        } catch (err) {
    ...
    if (audioOut.match("HTTP")) { //WEb Audio API
        ...
        switch (type) {
            case 'vGain':
                ...
                break;
            case '40Gain':
                pass40.gain.value = value;
                eqSlider.f40Hz[4] = value * 10
                if (audioOut.match("HTTP")){
                    //WebAudioAPI
                    if (filters[4]) {
                        filters[4].gain.value = ((slider.value - 50) / 5).toFixed(1);
                        console.log("WebAudioAPI 40Hz Gain="+filters[4].gain.value)
                    }
                }
                break;
            case '121Gain':
                pass121.gain.value = value;
                eqSlider.f121Hz[4] = value * 10
                break;
            case '547Gain':
                pass547.gain.value = value;
                eqSlider.f547Hz[4] = value * 10
                break;
            case '674Gain':
                pass674.gain.value = value;
                eqSlider.f674Hz[4] = value * 10
                break;
            case '818Gain':
                pass818.gain.value = value;
                eqSlider.f818Hz[4] = value * 10
                break;
            case '1503Gain':
                pass1503.gain.value = value;
                eqSlider.f1503Hz[4] = value * 10
                break;
            case '1605Gain':
                pass1605.gain.value = value;
                eqSlider.f1605Hz[4] = value * 10
                break;
            case '1722Gain':
                pass1722.gain.value = value;
                eqSlider.f1722Hz[4] = value * 10
                break;
            case '4160Gain':
                pass4160.gain.value = value;
...
function updateEqValue(slider,id){
    const db = ((slider.value - 50) / 5).toFixed(1);
...
function setEq(i) {
    ...
    if (audioOut.match("HTTP") && webAudioApi) {
        pass40.gain.value = eqSlider.f40Hz[i] / 10 //dB
        pass121.gain.value = eqSlider.f121Hz[i] / 10
        pass547.gain.value = eqSlider.f547Hz[i] / 10
        pass674.gain.value = eqSlider.f674Hz[i] / 10
        pass818.gain.value = eqSlider.f818Hz[i] / 10
        pass1503.gain.value = eqSlider.f1503Hz[i] / 10
        pass1605.gain.value = eqSlider.f1605Hz[i] / 10
        pass1722.gain.value = eqSlider.f1722Hz[i] / 10
        pass4160.gain.value = eqSlider.f4160Hz[i] / 10
...
function PlayRadioBars(id, i, stat, name) {
    ...
    if (!radioPlaying) {
        ...
        mediaElement.connect(filters[0]);
...
//------------------------------
//HTTP
async function PlayRadio(id, i, stat, name) {
    console.log("PlayRadio: " + id + i + ", " + stat + ", " + name)
    ...
    if (!radioPlaying) {
        ...
        showStationName = document.getElementById("Station")
        showStationName.innerHTML = stat //name
        console.log("radioPlaying false, start playing...")
        oldButtonObj = document.getElementById(id + i)
